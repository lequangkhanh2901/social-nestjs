...
export class SeenMessage1699459849177 implements MigrationInterface {
  name = 'SeenMessage1699459849177'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`conversation\` ADD \`unreadLastUsersId\` json NOT NULL`,
    )
    await queryRunner.query(
      `ALTER TABLE \`message\` ADD \`seen\` tinyint NOT NULL DEFAULT 1`,
    )
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE \`message\` DROP COLUMN \`seen\``)
    await queryRunner.query(
      `ALTER TABLE \`conversation\` DROP COLUMN \`unreadLastUsersId\``,
    )
  }
}
