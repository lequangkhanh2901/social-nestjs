...
export class UpdateMedia1696919770689 implements MigrationInterface {
  name = 'UpdateMedia1696919770689'
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`medias\` ADD \`relationType\` enum ('POST', 'COMMENT') NOT NULL DEFAULT 'POST'`,
    )
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`medias\` DROP COLUMN \`relationType\``,
    )
    await queryRunner.query(
