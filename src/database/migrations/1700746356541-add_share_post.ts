...
export class AddSharePost1700746356541 implements MigrationInterface {
  name = 'AddSharePost1700746356541'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`posts\` ADD \`isOrigin\` tinyint NOT NULL DEFAULT 1`,
    )
    await queryRunner.query(
      `ALTER TABLE \`posts\` ADD \`originPostId\` varchar(36) NULL`,
    )
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`posts\` DROP COLUMN \`originPostId\``,
    )
    await queryRunner.query(`ALTER TABLE \`posts\` DROP COLUMN \`isOrigin\``)
  }
}
