...
export class Report1700807310564 implements MigrationInterface {
  name = 'Report1700807310564'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`report\` (\`id\` varchar(36) NOT NULL, \`handled\` tinyint NOT NULL DEFAULT 0, \`reason\` enum ('PROPAGANDA_OF_VIOLENCE', 'TERRORISM', 'VIOLATE_THE_RULES', 'SENSITIVE_CONTENT_OF_CHILDREN') NOT NULL, \`note\` varchar(255) NOT NULL, \`status\` enum ('NONE', 'REJECTED', 'ACCEPTED') NOT NULL DEFAULT 'NONE', \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`userId\` varchar(36) NULL, \`userTargetId\` varchar(36) NULL, \`postId\` varchar(36) NULL, \`commentId\` int NULL, \`managerId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    )
    await queryRunner.query(
