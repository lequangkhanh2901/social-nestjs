...
export class ChangeUpdatedDateNotification1700332803549
  implements MigrationInterface
{
  name = 'ChangeUpdatedDateNotification1700332803549'
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`notification\` CHANGE \`updatedAt\` \`updatedAt\` datetime NOT NULL`,
    )
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`notification\` CHANGE \`updatedAt\` \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6)`,
    )
    await queryRunner.query(
      `ALTER TABLE \`notification\` DROP COLUMN \`userIds\``,
    )
