...
export class Notification1700311622648 implements MigrationInterface {
  name = 'Notification1700311622648'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`notification\` (\`id\` varchar(36) NOT NULL, \`type\` enum ('LIKE_MY_POST', 'LIKE_MY_COMMENT', 'NEW_POST_FROM_FRIEND', 'USER_COMMENTED_MY_POST', 'NEW_REQUEST_FRIEND', 'USER_ACCEPTED_REQUEST_FRIEND') NOT NULL, \`userIds\` json NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`userId\` varchar(36) NULL, \`userTagetId\` varchar(36) NULL, \`postId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    )
    await queryRunner.query(
