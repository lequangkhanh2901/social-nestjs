...
export class ViewStatusMessage1699698511603 implements MigrationInterface {
  name = 'ViewStatusMessage1699698511603'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`message\` CHANGE \`seen\` \`viewStatus\` tinyint NOT NULL DEFAULT '1'`,
    )
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`message\` DROP COLUMN \`viewStatus\``,
    )
    await queryRunner.query(
      `ALTER TABLE \`message\` ADD \`viewStatus\` enum ('SENT', 'RECEIVED', 'VIEWED') NOT NULL DEFAULT 'SENT'`,
    )
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`message\` ADD \`viewStatus\` tinyint NOT NULL DEFAULT '1'`,
    )
    await queryRunner.query(
      `ALTER TABLE \`message\` ADD CONSTRAINT \`FK_7cf4a4df1f2627f72bf6231635f\` FOREIGN KEY (\`conversationId\`) REFERENCES \`conversation\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    )
    ...
    await queryRunner.query(
      `ALTER TABLE \`message\` CHANGE \`viewStatus\` \`seen\` tinyint NOT NULL DEFAULT '1'`,
    )
  }
}
